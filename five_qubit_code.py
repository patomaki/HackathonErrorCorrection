...
from pyquil.gates import H, CNOT, CZ, Z, X, MEASURE
...
# #
# #
def alt_encode_five_to_one_quiltxt(code_indices,data_q_index,p):
    '''
    '''
    indices = [i for i in code_indices if i != data_q_index]
    enc_p = p
...
# #
# #
def qrng_quiltxt(code_indices,p):
    '''
        Return a quil plaintext which is
        p appended with Hadamard operations to qubits with indices
        "indices". It also acts as a transversal logical H.
        Input code_indices: qubit indices to apply Hadamards to
              p: pyquil program object
    '''
    qrng_p = p
    qrng_p.inst([H(q) for q in code_indices])
    return qrng_p
# #
# #
